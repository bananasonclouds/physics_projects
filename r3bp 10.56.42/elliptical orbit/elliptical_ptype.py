...
m3 = 0.01 * M_jupiter  # Planet (jupiter mass)
...
def run_runge(m1, m2, m3, times, dt):
    r = 6e11  # Initial distance between the stars (in meters) #2AU
    r_planet = 0.5e12 #4.5e10 * perturbation_factor_planet  # Initial distance of the planet from the center of mass OF BOTH STARS (in meters)
...
perturbation_factor_planet = 24 #generate_random_float() * 1
...
def orbital_period_years(semi_major, M1, M2):
    G = 6.6726 * pow(10, -11)  # m^3 kg^-1 s^-2
    seconds_in_year = 365.25 * 24 * 3600  # Approximation
    # the orbital period using Kepler's third law
    orbital_period_seconds =  np.sqrt((4 * np.pi**2 / (G * (M1 + M2))) * semi_major)

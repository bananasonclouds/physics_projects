...
total_time = 365.25 * 24 * 3600 * 10 # One year in seconds
times = np.arange(0, total_time, dt)

# Run the simulation using the runge_kutta function
x1s, y1s, x2s, y2s, x3s, y3s = [], [], [], [], [], []
for _ in times:
...
# Plot orbits
axs[0].plot(x1s, y1s, label='Star 1',color='blue')
axs[0].plot(x2s, y2s, label='Star 2',color='cyan')
axs[0].plot(x3s, y3s, label='Planet',color='red',linewidth=0.8)
axs[0].set_xlabel('X Position (m)',fontsize=14)
axs[0].set_ylabel('Y Position (m)',fontsize=14)
#axs[0].legend()
axs[0].axis('equal') 

# Set tick parameters to make ticks inside the plot
axs[0].tick_params(axis='x', direction='in', bottom=True, top=False, labelbottom=True)  # x-axis ticks
axs[0].tick_params(axis='y', direction='in', left=True, right=False, labelleft=True)    # y-axis ticks

# Plot acceleration of the planet
time = np.arange(0, len(x3s)*dt, dt)
axs[1].plot(time, acceleration(m1, m2, m3, np.array(x1s), np.array(y1s), np.array(x2s), np.array(y2s), np.array(x3s), np.array(y3s))[4], label='Planet Acceleration', color='green')
axs[1].set_xlabel('Time (s)')
axs[1].set_ylabel('Acceleration (ms$^{-2}$)')

#axs[1].legend()

def calculate_star_planet_separation(x1s, y1s, x2s, y2s, x3s, y3s):
    star1_planet_separation = np.sqrt((np.array(x1s) - np.array(x3s))**2 + (np.array(y1s) - np.array(y3s))**2)
    star2_planet_separation = np.sqrt((np.array(x2s) - np.array(x3s))**2 + (np.array(y2s) - np.array(y3s))**2)
    return star1_planet_separation, star2_planet_separation

def plot_star_planet_separation(time, star1_planet_separation, star2_planet_separation):
    plt.figure(figsize=(8, 6))
    plt.plot(time, star1_planet_separation, label='Star 1-Planet Separation', color='purple')
    plt.plot(time, star2_planet_separation, label='Star 2-Planet Separation', color='orange')
    plt.xlabel('Time (s)')
    plt.ylabel('Separation (m)')
    plt.title('Separation of Planet from Stars over Time')
    plt.legend()
    plt.grid(True)
...
# Calculate separation of planet from both stars
star1_planet_separation, star2_planet_separation = calculate_star_planet_separation(x1s, y1s, x2s, y2s, x3s, y3s)

# Plot separation of planet from both stars
plot_star_planet_separation(times, star1_planet_separation, star2_planet_separation)

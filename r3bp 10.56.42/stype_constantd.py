...
m3 = 6 * pow(10, 24)  # Planet (earth mass)

miu = m2 / (m1 + m2)  # mass ratio
print(miu)


# Acceleration function including the planet orbiting one of the stars
...
# Runge-Kutta function including the planet orbiting one of the stars
...
# Run simulation including the planet orbiting one of the stars
def run_runge(m1, m2, m3, perturbation_factor, t_max, dt):
    r = 3e11  # Initial distance between the stars (in meters)
    r_planet = 4.5e10 * perturbation_factor  # Initial distance of the planet from one of the stars (in meters)

    # Velocities for stable orbits
    v_stars = np.sqrt(G * (m1 + m2) / r) * perturbation_factor
    v_planet = np.sqrt(G * m2 / r_planet)  # Planet orbits around star 2
...
t_max = 365 * 24 * 3600 * 10  # Ten years
dt = 200  # Time step
